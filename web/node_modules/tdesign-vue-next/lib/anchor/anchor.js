...
import { c as off, i as isServer, b as getScrollContainer, o as on, d as getScroll, s as scrollTo } from '../_chunks/dep-a7922f61.js';
import 'lodash-es';
...
var _Anchor = defineComponent({
  ...
  setup: function setup(props2, _ref) {
    ...
    var handleScroll = function handleScroll() {
      ...
      var filters = [];
      var nextActive = "";
      var isScrollBottom = scrollContainer.value instanceof HTMLElement && Math.abs(scrollContainer.value.scrollTop + scrollContainer.value.clientHeight - scrollContainer.value.scrollHeight) < 2;
      ...
      if (isScrollBottom && links.value.length) {
        var currentActiveIndex = links.value.indexOf(active.value);
        var calcActiveIndex = links.value.indexOf(nextActive);
        if (currentActiveIndex > calcActiveIndex) {
          var anchor = getAnchorTarget(active.value);
          if (anchor) {
            var top = getOffsetTop(anchor, scrollContainer.value);
            if (scrollContainer.value instanceof HTMLElement) {
              var clientHeight = scrollContainer.value.clientHeight;
