...
var FILTER_INACTIVE_LEVEL = -1;
function checkOptionMatchKeyword(option, keyword) {
  if (!option.label || !keyword) return false;
  return option.label.toLowerCase().includes(keyword);
}
function isFilterLevelActive(level) {
  return level !== FILTER_INACTIVE_LEVEL;
}
function isFilterActive(filter) {
  if (filter === void 0) return false;
  if (typeof filter === "string") return Boolean(filter.trim());
  return true;
}
function filterOptions(nodes, filter, panelIndex) {
  if (typeof filter === "string") {
    var keyword = filter.trim().toLowerCase();
    if (!keyword) return nodes;
    return nodes.filter(function (node) {
      return checkOptionMatchKeyword(node, keyword);
    });
  }
  return nodes.filter(function (node) {
    return filter(node.data, panelIndex);
  });
}

export { FILTER_INACTIVE_LEVEL, checkOptionMatchKeyword, filterOptions, getCascaderValue, getFullPathLabel, getMultipleContent, getPanels, getSingleContent, getTreeValue, isEmptyValues, isFilterActive, isFilterLevelActive, isValueInvalid };
